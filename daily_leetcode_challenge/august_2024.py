...
Intuition
One brute-force approach is to consider each 3x3 subarray of the grid and manually check if each subarray satisfies the definition of a 3x3 magic square.

We iterate through the entire grid, examining each possible 3x3 subarray. For each subarray, we'll check each element to make sure that it is within allowed range and that it isn't a duplicate. Then, we verify that the sums of all three rows, three columns, and the two diagonals are equal. If all these conditions are met, then the subarray is a magic square.
"""
class Solution:
